...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class Circle {
        constructor(x, y, z, radius, color, xmom = 0, ymom = 0, friction = 1, reflect = 0, strokeWidth = 0, strokeColor = "transparent") {
            this.x = x
            this.y = y
            this.z = z
            this.alpha = 1
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class PolygonD {
        ...
        draw(nodes) {
            ...
            for(let t = 0;t<nodes.length;t++){
                this.nodes.push(nodes[t].center)
            }
    ...
    class Donut{
        constructor(x,y, color){
            ...
            this.angle1start = 0 //Math.random()*Math.PI*2
            this.angle1 = 0
            for(let k = 0;k<60;k++){
                this.angle2 = 0
        ...
        isPointInside(point){
            let link = new LineOP(this.center, point)
            if(link.hypotenuse() < 265){
                if(link.hypotenuse() > 130){
                this.hit = 1
                }
            }
        }
        ...
        for(let t = 1;t<this.points.length;t++){
            let link = new LineOP(this.points[t], this.points[t-1], "#0000FF", 1.5)
            link.target.y-=20
            link.draw()
        }
        // for(let t = 1;t<this.points.length;t++){
        ...
        // }
        // for(let t = 1;t<this.points.length;t++){
        //     let link = new LineOP(this.points[t], this.points[t-1], "#00FF00", 1.5)
        //     link.target.y+=20
        //     if(this.hit == 1){
        //         link.color = "#ff0000"
        //     }
    ...
    let dot = new Rectangle(1050, 1050, 9, 9, "white")
    ...
    function main() {
        ...
        // control(dot, 7)
        // dot.draw()
        let polynut = new PolygonD(0,0, 3, donuts[0].points[0].color, 1,1,1,1,1)
        polynut.draw(donuts)
        tick+=.01
        for(let t = 0;t<donuts.length;t++){
            // donuts[t].isPointInside(dot)
    

        if(flip == -1){
            donuts[t].center.x = 1050+(Math.cos(tick +t*(Math.PI/19)))*700
            donuts[t].center.y = 1050+Math.sin(tick +t*(Math.PI/19))*700
            spin = (new LineOP(new Point(1050, 1050),  donuts[t].center)).angle()
